...
// Mapa de mundo: panel MECÁNICO empotrado en la página (no una ventana
// flotante que bloquea) — se despliega encima de la escena de juego, que
// sigue viéndose y funcionando debajo mientras el mapa está abierto. Solo
// se dibuja el camino (puntos + línea) de la FASE ACTUAL, hacia el boss al
// que vas — trazado a mano por Carlos sobre el mapa real (ver
// config/worlds.ts) — las otras 4 zonas quedan como iconos de contexto,
// sin ruta, para no saturar el mapa. 100% reutilizable para otros mundos:
// solo cambian `mapBg`/`zones`/`paths` en config/worlds.ts.
export default function WorldMap({ currentScreen, bestScreen, isOpen, onClose }: Props) {
  const t = useT();
  ...
  return (
    <div className={`worldmap-inline panel worldmap-panel ${isOpen ? 'is-open' : 'is-closing'}`}>
